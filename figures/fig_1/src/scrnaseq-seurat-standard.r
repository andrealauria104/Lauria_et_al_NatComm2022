...
# paths ---
path_seurat_obj <- "data/seurat_obj.rds"
path_results    <- "seurat/results_seurat-standard" 

if(!dir.exists(path_results)) dir.create(path_results, recursive = T)

# global parameters ---
metadata_features <- c("Library_PreP_Batch","TimeGenotype","TimeGenotypeFull") # used for grouping in plots, character vector
...
scale_data_vars_to_regress <- c("Library_PreP_Batch","percent.mt")
...
clsutering_resolution = 1 # .8 seurat default
